...
export default function getNewRecentSearchHistory(
  ...
  const existingIndex = recentSearchHistory.findIndex(isDuplicated);
  // 중복하는 요소가 있는지 관리하는 변수입니다.
  const isNotDuplicated = existingIndex === -1;

  if (isNotDuplicated) {
    // 중복이 없다면 새롭게 객체를 생성하고, isFavorite는 False로 설정한 뒤에 최대 5개까지만 리턴합니다.
  ...
  return [
    recentSearchHistory[existingIndex],
    ...recentSearchHistory.filter((_, index) => index !== existingIndex),
  ];
}
