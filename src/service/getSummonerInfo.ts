...
export default async function getSummonerInfo(
  ...
  try {
    ...
    const accountRes: AxiosResponse<SummonerAcountData> = await axios.get(
      `/asia/riot/account/v1/accounts/by-riot-id/${name}/${tag}?api_key=${process.env.NEXT_PUBLIC_RIOT_API_KEY}`,
    );
    // 소환사 puuid입니다.
    ...
    const basicRes: AxiosResponse<SummonerBasicData> = await axios.get(
      `/${region}/lol/summoner/v4/summoners/by-puuid/${puuid}?api_key=${process.env.NEXT_PUBLIC_RIOT_API_KEY}`,
    );
